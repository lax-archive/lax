...
- trusted background imports
    - mathlib from its canonical repository, pinned to the selected
      environment's revision and present in **every** submission workspace.
      Importing it remains the author's choice — presence without import is
      inert.
...
- **Dependencies.** Besides mathlib, concept packages may require only
  ...
  from its canonical URL, pinned to the submission environment's revision.
