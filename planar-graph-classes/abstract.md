...
theorem records the equivalence for finite graphs. Each graph-class predicate
contains only its defining structure: planarity and superclass consequences
are stated separately as theorem cards. Proofs are supplied only where the
inclusion is an elementary projection or composition in the present
formalization.
