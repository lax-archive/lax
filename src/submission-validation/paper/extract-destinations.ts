...
//
//   node extract-destinations.js <pdf>
//
...
interface PdfDocumentLike {
  numPages: number;
  getPage(index: number): Promise<{ view: number[] }>;
...
async function main(pdfPath: string): Promise<ExtractedPdf> {
  ...
  try {
    ...
    return { pages: document.numPages, pageSizes, destinations, unknown };
  } finally {
...
if (pdfPath === undefined) {
  process.stderr.write("usage: extract-destinations <pdf>\n");
  process.exit(2);
}
main(pdfPath).then(
  (result) => {
