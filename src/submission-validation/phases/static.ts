...
import { LAX_GENERATED_FILES } from "../generated-files.js";
import type {
  PaperManifest,
  StaticPaper,
  StaticResult,
...
export function runStaticValidation(
  ...
  // One reading of the tree, shared by every check below that needs to know
  // what the archive will actually receive.
  const tree = readSubmissionTree(root);
  checkTrackedFiles(tree, findings, result.manifest?.paper !== undefined);
  ...
  for (const kind of ["concepts", "proofs"] as const) {
    ...
    checkPackageFiles(packageDir, kind, packageName, tree, findings);
  ...
  const walk = (relative: string): void => {
    ...
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        if (entry.name === ".git" || entry.name === LAX_GENERATED_FILES.buildTree) continue;
        walk(relativeEntry);
        continue;
      }
      if (!entry.isFile()) {
      ...
      if (relative === "" && folderReal === rootReal && LAX_GENERATED_FILES.root.includes(entry.name)) continue;
...
/**
 * Nothing lax generates may be committed: a tracked copy is rewritten or
 * deleted by the next build, so it can only ever disagree with the sources
 * beside it. The names come from the generated-files module rather than from a
 * list kept here, because this guard and the scaffold's `.gitignore` are two
 * halves of the same promise and the paper layer once extended only one.
 */
function checkTrackedFiles(tree: SubmissionTree, findings: FindingCollector, paperDeclared: boolean): void {
  for (const filename of tree.tracked) {
    ...
    } else if (LAX_GENERATED_FILES.anywhere.includes(basename)) {
      findings.violate("generated-files", `generated file must not be committed: ${filename}`);
    } else if (paperDeclared && LAX_GENERATED_FILES.paper.includes(filename)) {
      // the local build writes the compiled paper and its web bundle beside
      // build-output.json, at the submission root and nowhere else
      findings.violate("generated-files", `generated file must not be committed: ${filename}`);
    } else if (filename.split("/").includes(LAX_GENERATED_FILES.buildTree)) {
