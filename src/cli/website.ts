  ...
  for (const [index, root] of candidates.entries()) {
    if (!fs.existsSync(path.join(root, "dist", "sitegen", "generate.js"))) {
      if (index === 0 && fs.existsSync(root)) {
        console.warn("lax serve: downloaded Website renderer is incomplete; using the bundled fallback");
      }
      continue;
    }
    try {
      ...
      if (index === 0) {
        console.warn("lax serve: downloaded Website renderer is unusable; using the bundled fallback");
      }
    }
  }
  const detail = failures.length === 0 ? "no renderer installation was found" : failures.join("; ");
  throw new Error(
    "the lax-website page-builder is unavailable; reinstall the CLI or run `lax upgrade`: " + detail,
  );
}
