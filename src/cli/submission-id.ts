...
import { NEW_SUBMISSION_ID_PATTERN } from "../shared/constants.js";
import { validateNewSubmissionId } from "../shared/validation.js";

export function generateSubmissionId(draw: (minimum: number, maximum: number) => number = randomInt): string {
...
export function validateScaffoldIdentity(root: string, id: string): void {
  ...
  for (const filename of expected) {
    if (!fs.existsSync(filename)) {
      throw new Error(`submission id ${id} does not match the generated package layout; missing ${filename}`);
    }
  }
...
export function isNewSubmissionId(value: string): boolean {
  return NEW_SUBMISSION_ID_PATTERN.test(value);
}
