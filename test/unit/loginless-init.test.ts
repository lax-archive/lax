...
describe("loginless submission initialization", () => {
  it("freezes the complete pre-migration submission id allowlist", () => {
    expect([...LEGACY_SUBMISSION_IDS]).toEqual([
      ...
      "lax-55", "lax-56", "lax-57", "lax-58", "lax-59", "lax-60", "lax-61", "lax-62", "lax-65",
      "lax-66", "lax-67",
    ]);
  });
  ...
  it("scaffolds a six-digit manifest id without touching GitHub", async () => {
    ...
    await initializeSubmission(root, { title: "Local theorem" });
  ...
  it("verifies and records the issue binding for an allowlisted legacy submission", async () => {
    ...
    const fetchMock = installArchiveFetch("lax-3", legacyFiles);
    vi.stubGlobal("fetch", fetchMock);
    vi.spyOn(console, "log").mockImplementation(() => undefined);
  ...
  it("resumes an interrupted issue binding and preserves the commit stop", async () => {
    const root = scaffold();
    setManifestIssue(root, issue);
    process.env.LAX_POLL_INTERVAL_MS = "1";
    process.env.LAX_WORKFLOW_TIMEOUT_MS = "1000";
    const boundFiles = initialFiles(id, issue, alice, "2026-08-01T10:00:00Z");
    const fetchMock = installArchiveFetch(id, boundFiles, { availableAfterInitialization: true });
    vi.stubGlobal("fetch", fetchMock);
    vi.spyOn(console, "log").mockImplementation(() => undefined);
    ...
    expect(readLocalSubmissionManifest(root).issue).toEqual(issue);
    expect(
      fetchMock.mock.calls.filter(([, init]) => (init as RequestInit | undefined)?.method === "POST"),
    ).toHaveLength(0);
    expect(
      fetchMock.mock.calls.some(([url]) =>
        new URL(String(url)).pathname.endsWith("/issues/42/comments")),
    ).toBe(true);
  });

  it("creates and records the issue on first submit, then synchronizes local owners", async () => {
    const root = scaffold();
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      ...
      if (url.pathname === "/repos/lax-archive/lax-database") {
      ...
      if (url.pathname === "/repos/lax-archive/lax-database/git/trees/root-tree") {
        return json({
          truncated: false,
          tree: issueCreated
            ? [{ path: id, mode: "040000", type: "tree", sha: "submission-tree" }]
            : [],
        });
      }
      ...
      if (url.pathname.startsWith(blobPrefix)) {
        ...
        const content =
          name === "owner-list.json" && ownerCommand !== ""
            ? `${JSON.stringify({
                specVersion: "1",
  ...
  it("rekeys before creating an issue when the chosen id already exists", async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, _init?: RequestInit): Promise<Response> => {
      ...
      if (url.pathname === "/repos/lax-archive/lax-database/git/trees/root-tree") {
        return json({
          truncated: false,
          tree: [{ path: id, mode: "040000", type: "tree", sha: "old" }],
        });
      }
...
function installArchiveFetch(
  ...
  options: { availableAfterInitialization?: boolean } = {},
): ReturnType<typeof vi.fn> {
  let available = options.availableAfterInitialization !== true;
  return vi.fn(async (input: RequestInfo | URL): Promise<Response> => {
    ...
    if (url.pathname === "/repos/lax-archive/lax-database/git/trees/root-tree") {
      return json({
        truncated: false,
        tree: available
          ? [{ path: submissionId, mode: "040000", type: "tree", sha: "submission-tree" }]
          : [],
      });
    }
    if (url.pathname === "/repos/lax-archive/lax-database/git/trees/submission-tree") {
